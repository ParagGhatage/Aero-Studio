...
const TOOLS = [
  {
    ...
    description:
      "Drag-and-drop image organiser with albums, full-screen viewer, and local IndexedDB storage.",
    status: "ready",
    icon: (
  ...
  },
  {
    ...
    description:
      "Crop, rotate, and export images with a lightweight local editor.",
    status: "ready",
    icon: (
      <svg viewBox="0 0 28 28" fill="none" className="w-full h-full">
        <rect
  ...
  },
  {
    ...
    description:
      "Compress any image to desired size without losing visual fidelity.",
    status: "ready",
    icon: (
      <svg viewBox="0 0 28 28" fill="none" className="w-full h-full">
        <path
          d="M7 4 L17 4 L21 8 L21 24 L7 24 Z"
          stroke="currentColor"
          strokeWidth="1.5"
          strokeLinejoin="round"
        />
        ...
        />
        <polyline
          points="12,10 14,12 12,14 14,16 12,18"
          fill="none"
          stroke="currentColor"
          strokeWidth="1.5"
          strokeLinecap="round"
  ...
  },
  {
    ...
    description:
      "Resize and compress multiple images at once. Export to JPEG, PNG or WebP with quality control.",
    status: "soon",
    icon: (
      <svg viewBox="0 0 28 28" fill="none" className="w-full h-full">
        <rect
        ...
        />
        <path
          d="M21 11 L25 7 M25 7 H21 M25 7 V11"
          stroke="currentColor"
          strokeWidth="1.5"
          strokeLinecap="round"
          strokeLinejoin="round"
...
export default function ImagesHub() {
  ...
  return (
    ...
    <div className="bg-[#0D0D0D] h-dvh font-sans text-[#F5F0EB] flex flex-col overflow-hidden px-5 py-6 md:px-10 md:py-10 box-border">
      ...
      <Helmet>
  <title>Image Tools Workspace | Aero Studio</title>
  <meta name="description" content="A suite of local-first image tools. Crop, compress, and organize your photos securely in your browser with zero server uploads." />
  <link rel="canonical" href="https://aerostudio.xyz/images" />
  
  {/* Fall back to your main social preview image, or use a specific one if you create an og-images.jpg */}
          ...
          {TOOLS.map((tool) => {
            ...
            return (
              <div
                ...
                onClick={
                  ready ? () => navigate(`/images/${tool.id}`) : undefined
                }
              >
                <div className="relative w-[72px] h-[72px] rounded-[18px] bg-[#1a1a1a] border border-[#2a2a2a] flex items-center justify-center mb-2.5 shadow-lg">
                  <div className="w-8 h-8 text-[#f5f0eb]">{tool.icon}</div>
                  {!ready && (
