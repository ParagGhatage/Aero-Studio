...
export default function Albums({ onSelectAlbum }) {
  ...
  const isAllSelected =
    albums.length > 0 && selectedAlbums.size === albums.length;

  const toggleSelectAll = () => {
    setSelectedAlbums(
      isAllSelected ? new Set() : new Set(albums.map((a) => a.id)),
    );
  };
  ...
  const deleteSingleAlbum = async (album, e) => {
    ...
    if (
      window.confirm(
        `Delete "${album.name}"? Photos will be moved to "My Photos".`,
      )
    ) {
  ...
  const deleteSelectedAlbums = async () => {
    if (window.confirm(`Delete ${selectedAlbums.size} selected album(s)?`)) {
      ...
      const albumsToDelete = await db.albums
        .where("id")
        .anyOf(albumIds)
        .toArray();
    ...
    <div className="flex flex-col h-full w-full overflow-hidden">
      ...
      {albums.length > 0 && (
        <div className="flex justify-start items-center mb-[14px] shrink-0">
          ...
          >
            <div
              className={`w-[14px] h-[14px] border rounded-[3px] inline-flex items-center justify-center text-[10px] ${isAllSelected ? "bg-[#FF5F1F] border-[#FF5F1F]" : "border-[#888]"}`}
            >
              {isAllSelected && "✓"}
            </div>
      ...
      <div className="flex-1 overflow-y-auto min-h-0 pr-1">
        <div className="grid grid-cols-[repeat(auto-fill,minmax(132px,1fr))] md:grid-cols-[repeat(auto-fill,minmax(174px,1fr))] gap-3 md:gap-[14px] pb-20">
          ...
          >
            ...
            <div className="text-[#F5F0EB] font-semibold text-[13px]">
              Create New
            </div>
          </div>
          ...
          {albums.map((album) => {
            const imageCount = images.filter(
              (img) => img.album === album.name,
            ).length;
            const isSelected = selectedAlbums.has(album.id);
            const showCheckbox =
              isSelected || hoveredId === album.id || selectedAlbums.size > 0;
            ...
            return (
              <div
                ...
                className={`relative bg-[#141414] border rounded-lg px-4 pt-6 pb-5 flex flex-col items-center justify-center cursor-pointer aspect-square transition-all duration-200 ${
                  isSelected
                    ? "border-[#FF5F1F]"
                    : "border-[#2A2A2A] hover:border-[#FF5F1F]"
                }`}
                onClick={() =>
                  selectedAlbums.size > 0 &&
                  toggleSelect(album.id, { stopPropagation: () => {} })
                }
                onDoubleClick={() =>
                  selectedAlbums.size === 0 && onSelectAlbum(album.name)
                }
                onMouseEnter={() => setHoveredId(album.id)}
                onMouseLeave={() => setHoveredId(null)}
              >
                <Helmet>
                  <title>Image Albums & Organization | Aero Studio</title>
                  <meta
                    name="description"
                    content="Organize your local photos into private albums. Manage your image collections securely in your browser without cloud storage."
                  />
                  <link
                    rel="canonical"
                    href="https://aerostudio.xyz/images/gallery/albums"
                  />
                </Helmet>
                {showCheckbox && (
                  <div
                ...
                <div
                  className="w-[58px] h-[48px] mb-4 relative"
                  aria-hidden="true"
                >
